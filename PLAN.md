...
2. **(Workflow — requires Tyler's `ultracode` opt-in; ask before launching)** Build sections to the new design as a **fan-out, one section per agent behind Storybook** — `Header.tsx` (NEW sticky nav) · Hero (branch-graph SVG) · About · Skills (commit-rail SVG) · Work (`ProjectsSection`, tab toggle) · `ContactSection.tsx` (rename of `ReachOutSection`, links only).

> **⛔ CORRECTED 2026-07-15 — PR2 is NOT one PR. The fan-out lands as PER-SECTION PRs into `modernize-updates`.**
>
> This paragraph originally said PR2 was a single PR. That contradicted `.claude/rules/branch-safety.md`'s **"one risky change per PR, so any regression is bisectable"** — a full-page redesign is ~6 risky changes, and one PR would mean a single Chromatic review where *every* story diffs at once. The rules say a machine cannot sign off on that; at that size neither can a human.
>
> **Tyler's decision:** `chore/redesign-shared` lands first (i18n keys, `MainSection` band wrapper, the anchor-id contract), then one PR per section, each with its own readable Chromatic diff and its own a11y gate. A bad section is one revert.
>
> **Scaffolding status:** the token map + JetBrains Mono + the tab fix already shipped in **#22** (`c46544d`), so step 2's "shared scaffolding first" is partly done. What remains shared: the `MainSection` band wrapper, `SocialLinksList`, and the Header↔section anchor contract.

Shared scaffolding first (`MainSection` band wrapper, `SocialLinksList`, `src/components/ui/provider.tsx`, token map applied to theme) so agents build against a stable base; use worktree isolation since sections are built in parallel. Efficient because each section is an independent unit against a fixed design + token map. Fonts added via self-hosted `@fontsource/*` as CSS vars — extend the existing `--font-*` pattern with `--font-mono`. (**Corrected 2026-07-15:** this line previously said `next/font/google`, contradicting the decision at §"Fonts" below and #18, which switched away from it after build-time Google Fonts fetches intermittently ETIMEDOUTed on Vercel. Never `next/font/google`.) **✅ DONE** — `@fontsource/jetbrains-mono` (400/500) + `--font-mono` at `:root` in `_app.tsx`, `.storybook/fonts.css`, and `.storybook/preview.tsx`; `theme.tokens.fonts.mono` probed live.
